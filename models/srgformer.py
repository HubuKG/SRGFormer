...
class SRGFormer(GeneralRecommender):
    def __init__(self, config, dataset):
        super(SRGFormer, self).__init__(config, dataset)
    ...
    def forward(self):
        ...
        if self.v_feat is not None and self.t_feat is not None:
            ...
            glb, _ = self.gtLayer(self.norm_adj, embs)
            ...
            str = self.alpha * F.normalize(glb) + self.beta * F.normalize(ghe_embs)
            all_embs = lge_embs + str
        else:
            all_embs = cge_embs
    ...
    def calculate_loss(self, interaction):
        ua_embeddings, ia_embeddings, hyper_embeddings, mm_embeddings = self.forward()
        users = interaction[0]
        ...
        [v_embs, t_embs] = mm_embeddings
        v_u, v_i = torch.split(v_embs, [self.n_users, self.n_items], dim=0)
        t_u, t_i = torch.split(t_embs, [self.n_users, self.n_items], dim=0)
        ...
        mcl_loss = self.ssl_triple_loss(v_u[users], t_u[users], t_u) + self.ssl_triple_loss(v_i[pos_items],
                                                                                                  t_i[pos_items], t_i)
        batch_reg_loss = self.reg_loss(u_g_embeddings, pos_i_g_embeddings, neg_i_g_embeddings)
        loss = batch_bpr_loss + self.reg_weight * batch_reg_loss + self.cl_weight * batch_hcl_loss + self.gama * mcl_loss
